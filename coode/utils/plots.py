...
def plot(model, indices, model_outputs ,pose, dolp_gt,aolp_gt,rgb_gt, path, epoch, img_res, plot_nimgs, max_depth, resolution):
    # arrange data to plot
    batch_size, num_samples, _ = aolp_gt.shape

    network_object_mask = model_outputs['network_object_mask']
    points = model_outputs['points'].reshape(batch_size, num_samples, 3)

    dolp_render = model_outputs['dolp_render']
    dolp_render = dolp_render.reshape(batch_size, num_samples,1).repeat(1,1,3)
    aolp_render = model_outputs['aolp_render']
    aolp_render = aolp_render.reshape(batch_size, num_samples,1).repeat(1,1,3)
    s0_render = model_outputs['s0']
    s0_render = s0_render.reshape(batch_size, num_samples,1).repeat(1,1,3)
    s0_render = s0_render / 1.0
    s1_render = (model_outputs['s1'] + 1.0) / 2.0
    s1_render = s1_render.reshape(batch_size, num_samples,1).repeat(1,1,3)
    s2_render = (model_outputs['s2'] + 1.0) / 2.0
    s2_render = s2_render.reshape(batch_size, num_samples,1).repeat(1,1,3)

    normals = model_outputs['normals']
    normals = normals.reshape(batch_size,num_samples,3)
    ...
    normals_cam = -torch.ones(batch_size,num_samples,3).cuda().float()
    normals_cam[:,network_object_mask,] = rend_util.get_normals_cam(normals, pose)[:,network_object_mask,:]
    normals_cam = normals_cam.reshape(batch_size, num_samples, 3)
    ...
    aolp_gt = aolp_gt / torch.pi
    aolp_render = aolp_render / torch.pi
    rgb_gt = (rgb_gt + 1.0) / 2.0
    plot_images(aolp_render, aolp_gt.cuda(), path, 'aolp', epoch, plot_nimgs, img_res)
    plot_images(dolp_render, dolp_gt.cuda(), path, 'dolp',epoch, plot_nimgs, img_res)
    plot_images(s0_render, rgb_gt.cuda(), path, 's0',epoch, plot_nimgs, img_res)
    plot_images(s1_render, s1_render, path, 's1',epoch, plot_nimgs, img_res)
    plot_images(s2_render, s2_render, path, 's2',epoch, plot_nimgs, img_res)

    # plot depth maps
    plot_depth_maps(depth, path, epoch, plot_nimgs, img_res)

    # plot normals
    plot_normals(normals=normals_cam,path=path,epoch=epoch,img_res=img_res)
...
def get_surface_high_res_mesh(sdf, resolution=100):
    ...
    print('vecs',vecs)
    if torch.det(vecs.detach().cpu()) < 0:
...
def plot_images(rgb_points, ground_true, path, suffix,epoch, plot_nrow, img_res):
    # 输入的图像必须为(0,1)
    ...
    img = Image.fromarray(tensor)
    img.save('{0}/{1}_rendering_{2}.png'.format(path, suffix,epoch))

def plot_normals(normals,path,epoch,img_res):
    output_vs_gt_plot = lin2img(normals, img_res)
    ...
    scale_factor = 127.5
    eps = 1e-10
    tensor = np.clip(tensor, -1.0 + eps, 1.0 - eps)
    tensor = ((tensor+1) * scale_factor).astype(np.uint8)

    img = Image.fromarray(tensor)
    img.save('{0}/normal_{1}.png'.format(path, epoch))
def plot_intensitys(intensity,path,suffix,epoch,img_res):
    # intensity: (0,1)
    reflection_plot = lin2img(intensity, img_res)
    # reflection_plot = reflection_plot[0,0,:,:].detach().cpu().numpy()
    tensor = torchvision.utils.make_grid(reflection_plot.repeat(1, 3, 1, 1),
                                         scale_each=False,
                                         normalize=False,
                                         nrow=1).cpu().detach().numpy()
    tensor = tensor.transpose(1, 2, 0)
    ...
    img = Image.fromarray(tensor)
    img.save('{0}/intensity_{1}_{2}.png'.format(path, suffix, epoch))
